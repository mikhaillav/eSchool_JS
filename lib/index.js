...
class eSchool {
    ...
    async getSessionId() {
        ...
        return axios_1.default
            ...
            throw new error_1.eSchoolError("Failed to parse token from cookies.", "login", 0);
        })
            .catch((e) => {
            switch (e.response?.data) {
                case 1:
                    throw new error_1.eSchoolError("Got code: 1. Login/password error.", "login", 1);
                case 3:
                    throw new error_1.eSchoolError("Got code: 3. Need to solve captcha.", "login", 3);
                case 4:
                    throw new error_1.eSchoolError("Got code: 4. The account is blocked.", "login", 4);
                default:
                    throw new error_1.eSchoolError(`Failed to handle error response from eSchool. Got data: ${e.response?.data}.`, "login");
            }
        });
    }
    /**
